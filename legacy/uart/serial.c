...
static void (*user_uart_rx[2])(void);
...
static void uart0_isr(void) {
  ...
  if(0x01 & isr_cause) {
    if(user_uart_rx[0]) user_uart_rx[0]();
  }
...
static void uart1_isr(void) {
  ...
  if(0x01 & isr_cause) {//If it's an RXE int...
    if(user_uart_rx[1]) user_uart_rx[1]();    //Call the user function to do something about it
  }
  //Acknowledge the VIC
  VICVectAddr = 0;
}
