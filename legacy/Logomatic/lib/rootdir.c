...
uint8_t find_file_in_dir(struct fat_fs_struct* Lfs, struct fat_dir_struct* Ldd, const char* name, struct fat_dir_entry_struct* Ldir_entry)
{
    while(fat_read_dir(Ldd, Ldir_entry))
    {
        if(strcmp(Ldir_entry->long_name, name) == 0)
        {
            fat_reset_dir(Ldd);
            return 1;
        }
...
struct fat_file_struct* open_file_in_dir(struct fat_fs_struct* Lfs, struct fat_dir_struct* Ldd, const char* name)
{
    struct fat_dir_entry_struct file_entry;
    if(!find_file_in_dir(Lfs, Ldd, name, &file_entry))
        return 0;

    return fat_open_file(Lfs, &file_entry);
}
