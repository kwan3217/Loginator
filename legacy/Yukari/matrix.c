...
int mxt(matrix* c, matrix* a, matrix* bt) {
  ...
  for(int i=0;i<c->row;i++) {
    for(int j=0;j<c->col;j++) {
      ...
      for(int k=0;k<a->col;k++) {
        int aidx=idx(a,i,k);
        int btidx=idx(bt,j,k);
        c->data[cidx]+=a->data[aidx]*bt->data[btidx];
      }
    }
  }
  return 0;
...
//Performs C=A'*b. Can't be in place.
int mts(matrix* c, matrix* at, fp b) {
  //check dimension constraints
#ifdef CHECK_COMPAT
  if(at==c) return -1; //Can't do this in place
  if(c->row!=at->col) return -2; //Check factor matrix compatibility
  if(c->col!=at->row) return -3;
#endif
  for(int i=0;i<c->row;i++) {
    for(int j=0;j<c->col;j++) {
      int cidx=idx(c,i,j);
      int atidx=idx(at,j,i);
      c->data[cidx]=at->data[atidx]*b;
    }
  }
  return 0;
}
