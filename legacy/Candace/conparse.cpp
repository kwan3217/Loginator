...
#define LOGCON_NAME "CandaceCon.txt"

//actual definitions of config parameters
int uartMode[2]={1,1};
int baud[2]={9600,57600};
char trigger[2]={'$','$'};
char frameEnd[2]={'\n','\n'};
int rawSize[2]={64,64};
int adcFreq=100;
...
const logcon_t fTable[]={
    ...
    {"UART1 baud",   "57600",f_uartBaud1,   NULL},          //Baud rate, or Auto
...
int readLogCon(void) {
  ...
  if(root_file_exists(LOGCON_NAME)) {
    ...
    len = fat16_read_file(&fd, stringBuf, 512);
    if(len<0) return -1;
    fat16_close_file(&fd);
  } else {
    ...
    fat16_write_file(&fd, stringBuf, len);
    fat16_close_file(&fd);
    sd_raw_sync();
  }
