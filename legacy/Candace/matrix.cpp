...
#ifdef CHECK_COMPAT
  if(&a==this) {mxerrno=-1;return;} //Can't do this in place
  if(&b==this) {mxerrno=-2;return;}
  if(a.cols!=b.rows) {mxerrno=-3;return;} //Check factor matrix compatibility
  if(  rows!=a.rows) {mxerrno=-4;return;} //Check product matrix size
  if(  cols!=b.cols) {mxerrno=-5;return;}
  mxerrno=0;
#endif
...
#ifdef CHECK_COMPAT
  if(&a==this) {mxerrno=-1;return;} //Can't do this in place
  if(&bt==this) {mxerrno=-2;return;}
  if(a.cols!=bt.cols) {mxerrno=-3;return;} //Check factor matrix compatibility
  if(  rows!=a.rows)  {mxerrno=-4;return;} //Check product matrix size
  if(  cols!=bt.rows) {mxerrno=-5;return;}
  mxerrno=0;
#endif
  for(int i=0;i<rows;i++) {
    for(int j=0;j<cols;j++) {
...
void matrix::mts(matrix& at, fp b) {
  //check dimension constraints
#ifdef CHECK_COMPAT
  if(&at==this) {mxerrno=-1;return;} //Can't do this in place
  if(rows!=at.cols) {mxerrno=-2;return;} //Check factor matrix compatibility
  if(cols!=at.rows) {mxerrno=-3;return;}
  mxerrno=0;
#endif
  for(int i=0;i<rows;i++) {
    for(int j=0;j<cols;j++) {
...
#ifdef CHECK_COMPAT
  if(a.rows!=1) {mxerrno=-1;return 0;};
  if(b.cols!=1) {mxerrno=-2;return 0;};
  mxerrno=0;
#endif
  fp c=0;
...
#ifdef CHECK_COMPAT
  if(a.rows!=1) {mxerrno=-1;return 0;}
  if(bt.rows!=1) {mxerrno=-2;return 0;}
  mxerrno=0;
#endif
  fp c=0;
  for(int i=0;i<a.cols;i++) {
...
matrix& matrix::operator+=(matrix& b) {
  //check dimension constraints
#ifdef CHECK_COMPAT
  if(b.rows!=rows) {mxerrno=-1;return *this;}
  if(b.cols!=cols) {mxerrno=-2;return *this;}
  mxerrno=0;
#endif
...
void matrix::vcross(matrix& va, matrix& vb) {
  //check dimension constraints
#ifdef CHECK_COMPAT
  if(&va==this) {mxerrno=-1;return;} //Can't do this in place
  if(&vb==this) {mxerrno=-2;return;}
  if(va.rows*va.cols!=3) {mxerrno=-3;return;}
  if(vb.rows*vb.cols!=3) {mxerrno=-4;return;}
  if(   rows*   cols!=3) {mxerrno=-5;return;}
  mxerrno=0;
#endif
...
#ifdef CHECK_COMPAT
  if(this==&m) {mxerrno=-1;return;} //Doesn't even make sense to do in place
  if(m.rows!=3 || m.cols!=3) {mxerrno=-2;return;} //Need a 3x3 matrix.
  if(rows*cols<4+ofs)        {mxerrno=-3;return;} //Just check that the vector
