...
static void tx_serial_fill_fifo(unsigned int port) {
  unsigned int len=FIFO_DEPTH;
  if(len>tx_len[port][0]) len=tx_len[port][0];
  if(port==0) {
    for(unsigned int i=0;i<len;i++) U0THR=*(tx_ptr[0][0]+i);
  } else {
    for(unsigned int i=0;i<len;i++) U1THR=*(tx_ptr[1][0]+i);
  }
  tx_len[port][0]-=len;
  tx_ptr[port][0]+=len;
  if(tx_len[port][0]==0) {
    ...
    tx_ptr[port][QUEUE_DEPTH-1]=(char*)NULL;
...
void tx_serial_block(int port, char* data, unsigned int len) {
  if(len==0) return;
  if(port==0) {
    for(unsigned int i=0;i<len;i++) putc_serial0(data[i]);
  } else {
    for(unsigned int i=0;i<len;i++) putc_serial1(data[i]);
  }    
}
...
static user_uart_rx_t user_uart_rx[2];
...
//== Port setup
void setup_serial(int port, int setbaud, user_uart_rx_t Luser_uart_rx) {
