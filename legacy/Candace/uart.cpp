...
inline static int isFrameEndNMEA(int port) {
  return (uartbuf[port].peekHead(-1)==frameEnd[port]);
}

inline static int isFrameEndSiRF(int port) {
  return (uartbuf[port].peekHead(-1)==0xB3 && uartbuf[port].peekHead(-2)==0xB0);
}
...
inline static int isFrameEnd(int port) {
  ...
  return uartbuf[port].unreadylen()<rawSize[port];
}
...
static void UARTISR(int port) {
  ...
  while(status & 0x01) {
    ...
    if(uartInPkt[port] || isFrameStart(port,next)) { //If already in the frame, or this character starts it
      ...
      uartbuf[port].fill(next);
      if(isFrameEnd(port) || uartbuf[port].unreadylen()>=912) {
...
void setup_uart(int port, int setbaud, int want_ints) {
  if(want_ints) {
    setup_serial(port,setbaud,UARTISR);
  } else {
    setup_serial(port,setbaud);
  }
...
static int countCharBaud(int port, int trybaud, unsigned int* chars, unsigned int* errors) {
  ...
  *errors=0;
  ...
  while((T0TC-start)<PCLK) {
    ...
    if(status & 0x01) {
      volatile char junk __attribute__ ((unused))=URBR(port);
      (*chars)++;
...
int autobaud(int port) {
  int lighton=0;
  unsigned int leastErrors=99999;
  int leastBaud=-1;
  ...
  //No speed was perfect, use the best speed
  if(leastBaud>=0) {
