...
const char versionString[]="Candace Rocketometer v0.01 "__DATE__" "__TIME__"MDT";
...
static void writeFirmware(void) {
  if(writeFirm==2) {
    ...
    // Read the file contents, and write it to flash
    while( (read=fat16_read_file(&fd,readbuf,READBUFSIZE)) > 0 )  {
...
//  init_VIC();
  ...
  for(int i=0;i<3;i++) fn[i+9]=PktWriter->ext[i];
  ...
  int oldMEMMAP=MEMMAP;
  MEMMAP=0x01;
  fillPktStart(sdBuf,PT_I2C);
  fillPktString(sdBuf,"Int table");
  sdBuf.dataDec=0;
  sdBuf.dataDigits=8;
  fillPktInt(sdBuf,MEMMAP);
  fillPktInt(sdBuf,0x00000000);
  fillPktBlock(sdBuf,(const char*)0x0,64);
  fillPktFinish(sdBuf);
  drainToSD(sdBuf);
  MEMMAP=oldMEMMAP;
  ...
  sdBuf.dataDigits=8;
  fillPktInt(sdBuf,MEMMAP);
  fillPktInt(sdBuf,0x10000);
  fillPktBlock(sdBuf,(const char*)0x10000,64);
  fillPktFinish(sdBuf);
  drainToSD(sdBuf);
  ...
  sdBuf.dataDigits=8;
  fillPktInt(sdBuf,MEMMAP);
  fillPktInt(sdBuf,0x40000000);
  fillPktBlock(sdBuf,(char*)0x40000000,64);
  fillPktFinish(sdBuf);
  drainToSD(sdBuf);
