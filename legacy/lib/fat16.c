...
static char fat16_read_header(struct fat16_fs_struct* fs);
static char fat16_read_root_dir_entry(const struct fat16_fs_struct* fs, uint16_t entry_num, struct fat16_dir_entry_struct* dir_entry);
static char fat16_read_sub_dir_entry(const struct fat16_fs_struct* fs, uint16_t entry_num, const struct fat16_dir_entry_struct* parent, struct fat16_dir_entry_struct* dir_entry);
static char fat16_dir_entry_seek_callback(char* buffer, uint32_t offset, void* p);
static char fat16_dir_entry_read_callback(char* buffer, uint32_t offset, void* p);
static char fat16_interpret_dir_entry(struct fat16_dir_entry_struct* dir_entry, const char* raw_entry);
...
static char fat16_free_clusters(const struct fat16_fs_struct* fs, uint16_t cluster_num);
static char fat16_terminate_clusters(const struct fat16_fs_struct* fs, uint16_t cluster_num);
static char fat16_write_dir_entry(const struct fat16_fs_struct* fs, const struct fat16_dir_entry_struct* dir_entry);

static char fat16_get_fs_free_callback(char* buffer, uint32_t offset, void* p);
 ...
 */
char fat16_read_header(struct fat16_fs_struct* fs) {
    if(!fs) return 0;
 ...
 */
char fat16_read_root_dir_entry(const struct fat16_fs_struct* fs, uint16_t entry_num, struct fat16_dir_entry_struct* dir_entry) {
 ...
 */
char fat16_read_sub_dir_entry(const struct fat16_fs_struct* fs, uint16_t entry_num, const struct fat16_dir_entry_struct* parent, struct fat16_dir_entry_struct* dir_entry)
{
 ...
 */
char fat16_dir_entry_seek_callback(char* buffer, uint32_t offset, void* p)
{
 ...
 */
char fat16_dir_entry_read_callback(char* buffer, uint32_t offset, void* p)
{
 ...
 */
char fat16_interpret_dir_entry(struct fat16_dir_entry_struct* dir_entry, const char* raw_entry)
{
 ...
 * \see fat16_read_dir
 */
char fat16_get_dir_entry_of_path(struct fat16_fs_struct* fs, const char* path, struct fat16_dir_entry_struct* dir_entry)
{
 ...
 */
char fat16_free_clusters(const struct fat16_fs_struct* fs, uint16_t cluster_num)
{
    #if FAT16_WRITE_SUPPORT
        if(!fs || cluster_num < 2)
            return 0;
 ...
 */
char fat16_terminate_clusters(const struct fat16_fs_struct* fs, uint16_t cluster_num)
{
    #if FAT16_WRITE_SUPPORT
        if(!fs || cluster_num < 2)
            return 0;
 ...
 */
int16_t fat16_read_file(struct fat16_file_struct* fd, char* buffer, uint16_t buffer_len)
{
    /* check arguments */
 ...
 * \see fat16_read_file
 */
int16_t fat16_write_file(struct fat16_file_struct* fd, const char* buffer, uint16_t buffer_len) {
 ...
 * \returns 0 on failure, 1 on success.
 */
char fat16_seek_file(struct fat16_file_struct* fd, int32_t* offset, char whence)
{
    if(!fd || !offset)
        return 0;
 ...
 * \returns 0 on failure, 1 on success.
 */
char fat16_resize_file(struct fat16_file_struct* fd, uint32_t size)
{
    #if FAT16_WRITE_SUPPORT
        if(!fd)
            return 0;
 ...
 * \see fat16_reset_dir
 */
char fat16_read_dir(struct fat16_dir_struct* dd, struct fat16_dir_entry_struct* dir_entry)
{
    if(!dd || !dir_entry)
        return 0;
 ...
 * \see fat16_read_dir
 */
char fat16_reset_dir(struct fat16_dir_struct* dd)
{
    if(!dd)
        return 0;
...
char fat16_set_dir(struct fat16_dir_struct* dd, uint16_t offset)
{
    if(!dd)
        return 0;
 ...
 * \returns 0 on failure, 1 on success.
 */
char fat16_write_dir_entry(const struct fat16_fs_struct* fs, const struct fat16_dir_entry_struct* dir_entry)
{
    #if FAT16_WRITE_SUPPORT
        if(!fs || !dir_entry)
            return 0;
        ...
        char name_len = strlen(dir_entry->long_name);
        char lfn_entry_count = (name_len + 12) / 13;
        char buffer[32];
        ...
        if(name_ext)
        {
            ...
            char name_ext_len = strlen(name_ext);
        ...
        else
        {
            ...
            /* Minimize 8.3 name clashes by appending
                                             ...
                                             */
            char num = dir_entry->cluster & 0xff;
...
/*			current_time = GPS_get_time();
			...
			char c_hour, c_minute, c_second;
			char c_year, c_month, c_day;
 ...
 */
char fat16_create_file(struct fat16_dir_struct* parent, const char* file, struct fat16_dir_entry_struct* dir_entry)
{
    #if FAT16_WRITE_SUPPORT
        ...
        char free_dir_entries_needed = strlen(file) / 13 + 1 + 1;
        char free_dir_entries_found = 0;
 ...
 */
char fat16_delete_file(struct fat16_fs_struct* fs, struct fat16_dir_entry_struct* dir_entry)
{
    #if FAT16_WRITE_SUPPORT
        if(!fs || !dir_entry)
            return 0;
 ...
 */
char fat16_get_fs_free_callback(char* buffer, uint32_t offset, void* p)
{
    struct fat16_usage_count_callback_arg* count_arg = (struct fat16_usage_count_callback_arg*) p;
    char buffer_size = count_arg->buffer_size;
    char i;
...
char find_file_in_dir(struct fat16_fs_struct* fs, struct fat16_dir_struct* dd, const char* name, struct fat16_dir_entry_struct* dir_entry)
{
    while(fat16_read_dir(dd, dir_entry))
    {
...
int open_file_in_dir(struct fat16_file_struct* fd, struct fat16_fs_struct* fs, struct fat16_dir_struct* dd, const char* name) {
    ...
    if(!find_file_in_dir(fs, dd, name, &file_entry)) return -27;
