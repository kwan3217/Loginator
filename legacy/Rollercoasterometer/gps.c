...
void displayCoords(void) {
  ...
  clearDisplay();
  if(GPShasPos) {
    unsigned int alat=abs(lat);
    unsigned int alon=abs(lon);
    putc_serial0(lat>=0?'N':'S');
    putc_serial0(' ');
    to0Dec(buf,alat/10000000,2);
    putstring_serial0(buf);
    putc_serial0('.');
    to0Dec(buf,alat%10000000,7);
    putstring_serial0(buf);
    putstring_serial0("  ");
    to0Dec(buf,countBits(satMask),2);
    putstring_serial0(buf);
    putc_serial0(lon>=0?'E':'W');
    to0Dec(buf,alon/10000000,3);
    putstring_serial0(buf);
    putc_serial0('.');
    to0Dec(buf,alon%10000000,7);
    putstring_serial0(buf);
    putstring_serial0("  ");
    to0Dec(buf,satNum,2);
    putstring_serial0(buf);
  } else {
    putstring_serial0("No position");
  }
}
