...
int16_t ax,ay,az,bx,by,bz,gx,gy,gz;
...
void initAcc() {
  acc.begin();
  Serial.println("ADXL345 started");
  uint8_t accId=gyro.whoami();
  Serial.print("L3G4200D identifier (should be '345'): ");
...
void setup() {
  ...
  worked=config.begin();
  if(worked) ccsds.finish(0x22); else ccsds.forget();
  sdStore.drain(); 
