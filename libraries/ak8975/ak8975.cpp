...
void AK8975::startMeasurementCore() {
  port->beginTransmission(partAddress);
  port->write(0x0A); //Start at register 2
...
void AK8975::finishMeasurementCore() {
  ...
  port->requestFrom(partAddress, 8);
...
bool AK8975::fillConfig(Packet& ccsds) {
  #include "write_packet_akcfg.inc"
  return true;
}
