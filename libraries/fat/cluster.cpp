...
bool Cluster::begin() {
  if(!p.read(0,bpb,510,2)) return false;
  if(bpb[0]!=0x55) return false;
  if(bpb[1]!=0xAA) return false;
  if(!p.read(0,bpb,0x0B,sizeof(bpb))) return false;
...
uint32_t Cluster::readTable(uint32_t cluster) {
  if(tableEntrySize!=12) {
    ...
    uint32_t result=0;
    p.read(entrySector,(char*)&result,entryOffset,tableEntrySize/8);
...
bool Cluster::writeTable(uint32_t cluster, char* buf, uint32_t entry) {
  if(tableEntrySize!=12) {
    uint32_t sectorsPerTable, entrySector, entryOffset;
    calcTableCluster(cluster, sectorsPerTable, entrySector, entryOffset);
    if(!p.read(entrySector,buf)) return false;
    ...
    for(int i=0;i<numTables;i++) if(!p.write(entrySector+i*sectorsPerTable,buf)) return false;
    return true;
  } else {
    return false;
  }
...
uint32_t Cluster::findFreeCluster(uint32_t startCluster) {
  for(uint32_t i=startCluster+1;i<numClusters+2;i++) if(readTable(i)==0) return i;
  for(uint32_t i=2;i<=startCluster;i++) if(readTable(i)==0) return i;
  return BAD;
}
