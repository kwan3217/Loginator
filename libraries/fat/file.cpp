...
bool File::create(const char* filename, char* buf,uint32_t dir_cluster) {
  ...
  de.setCluster(cl);
  if(!c.writeTable(cl,buf,c.EOF)) FAIL(c.errno*100+10);
  return true;
}
...
bool File::sync(char* buf) {
  if(!de.writeBack(buf)) FAIL(de.errno*100+8);
  return true;
}
...
//  uint32_t write_cluster=c.BAD;
  if(de.size==0) {
    ...
    if(c.BAD==next_cluster) FAIL(c.errno*100+12);
    sector=0;
    if(!c.write(next_cluster,sector,buf)) FAIL(c.errno*100+13);
    //Now the buffer is ours
    if(!c.writeTable(cluster,buf,next_cluster)) FAIL(c.errno*100+14);
    if(!c.writeTable(next_cluster,buf,c.EOF)) FAIL(c.errno*100+15);
    cluster=next_cluster;
    sector++;
    de.size+=c.sectorSize();
    if(!sync(buf)) FAIL(errno*100+16);
  } else {
    if(!c.write(cluster,sector,buf)) FAIL(c.errno*100+17);
    sector++;
    de.size+=c.sectorSize();
  }
  return true;
...
bool File::wipeChain(char* buf) {
  ...
  while(cluster!=0 && cluster!=c.EOF) {
    uint32_t next_cluster=c.readTable(cluster);
    if(next_cluster!=0) if(!c.writeTable(cluster,buf,0)) FAIL(c.errno*100+18);
    cluster=next_cluster;
  }
  return true;
}
...
bool File::remove(const char* filename, char* buf,uint32_t dir_cluster) {
  if(!de.find(filename,dir_cluster)) FAIL(de.errno*100+19);
  de.entry[0]=0xE5;
  if(!de.writeBack(buf)) FAIL(de.errno*100+20);
  if(!wipeChain(buf)) FAIL(errno*100+21);
  return true;
}
