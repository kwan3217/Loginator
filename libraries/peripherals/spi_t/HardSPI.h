...
//Send a string to the slave, block until finished
    void tx_block(int p0, const char* data, int count) {select_cs(p0);static_cast<T*>(this)->tx_block(data,count);deselect_cs(p0);};
...
//Receive a string from the slave, block until finished
    void rx_block(int p0, uint8_t tx, char* data, int count) {select_cs(p0);static_cast<T*>(this)->rx_block(tx,data,count);deselect_cs(p0);};
...
class HardSPI1:public HardSPI<HardSPI1> {
  public:
    HardSPI1() {};
    ...
    using HardSPI::tx_block;
    using HardSPI::rx_block;
};

class HardSPI0:public HardSPI<HardSPI0> {
  public:
    HardSPI0() {};
    /*virtual*/ void begin(unsigned int freq, unsigned char CPOL, unsigned char CPHA);
    /*virtual*/ void setfreq(unsigned int freq);
